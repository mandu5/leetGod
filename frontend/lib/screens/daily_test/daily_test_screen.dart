...
class _DailyTestScreenState extends State<DailyTestScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<DailyTestProvider>(context, listen: false).loadDailyTest();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('오늘의 리트 모의고사'),
        backgroundColor: Colors.transparent,
        elevation: 0,
      ),
      body: Consumer<DailyTestProvider>(
        builder: (context, provider, child) {
          if (provider.isLoading) {
            return const Center(child: CircularProgressIndicator());
          }

          if (provider.dailyTest == null) {
            ...
            return const Center(child: Text('오늘의 리트 모의고사를 불러오는 중...'));
          }

          final questions = provider.dailyTest!.questions;
  ...
  void _submitDailyTest() async {
    try {
      final provider = Provider.of<DailyTestProvider>(context, listen: false);
      final result = await provider.submitDailyTest(_answers);
      
      if (mounted) {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => DailyTestResultScreen(result: result),
          ),
        );
      }
    } catch (e) {
